...
export const isAuth = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try{
        ...
        if(users.length === 0) {
            res.status(401).json({ message: "User assosiated with this token no longer exists" });
            return;
        }
        ...
        res.status(401).json({ message: "Authentication Failed. Please login again", error });
    }
